//! Thin CLI wrapper around `fg_t1k_core::genotyper` (the Rust port of `genotyper`,
...
//! `SelectAllelesForGenes`, output formatting) is either in `fg_t1k_core::genotyper` or -- for
...
//! header comment via [`fg_t1k_core::genotyper::AlleleRef::new`], and computes each allele's
...
/// (`fg_t1k_core::genotyper::Genotyper::init_allele_info`'s
