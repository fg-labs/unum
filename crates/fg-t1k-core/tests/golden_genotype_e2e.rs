//! End-to-end golden-file test for the `genotype` capstone, converted from the
//! retired `fg-t1k-sys` `diff_genotype_e2e.rs` FFI/subprocess differential (see
//! `tests/common/mod.rs`). Runs a from-scratch reimplementation of the
//! `genotype` driver (calling `fg_t1k_core::genotyper` directly -- the core
...
/// `fg_t1k_core::genotyper` directly.
