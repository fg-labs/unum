        ...
        let (frag_len, mut frag_stdev) = if mate_paired {
            ...
            if k == 0 {
                ...
                for &d in &mate_diff[..k] {
                    let v = d + i64::from(read_len);
                    sum += v;
                    sumsq += v * v;
                }
                let k_i64 = i64::try_from(k).unwrap_or(i64::MAX);
                let frag_len_i64 = sum / k_i64;
                #[allow(clippy::cast_precision_loss)]
                let variance = (sumsq / k_i64 - frag_len_i64 * frag_len_i64) as f64;
                #[allow(clippy::cast_possible_truncation)]
                let stdev = variance.sqrt() as i32;
                let frag_len = i32::try_from(frag_len_i64).unwrap_or(i32::MAX);
                (frag_len, stdev)
            }
        } else {
            (read_len, 0)
