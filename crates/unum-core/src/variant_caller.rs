...
//! reassignment) lives in `crates/unum/src/stages/analyze.rs` (the CLI
//! layer), matching `stages/genotype.rs`'s own split of "core in
//! `unum-core`, driver in the CLI crate".
//!
//! # Scope: the 6b slice
//!
...
//! `fragment_assignment` the real `unum analyze` driver builds has already
...
//! testability without requiring the full `unum analyze` driver to
...
impl VariantCaller {
                ...
                if filter_low_qual && !bv.is_good_assignment(o.match_cnt) {
                    ...
                    if rb != b'N' {
                        if let Some(nuc_idx) = nuc_to_num(rb) {
                            ...
                            // DIVERGENCE from T1K (see docs/DIVERGENCES.md):
                            // T1K `VariantCaller.hpp:147` does `count[nucIdx] +=
                            // 1` unconditionally, making `count` identical to
                            // `unweighted_count` and letting the align-info-only
                            // pass (`update_type == 1`, `weight = 0`) and
                            // fractional multi-allele assignments contribute
                            // full support. `unum` accumulates `weight` instead,
                            // so `count` is the genuinely weighted tally its name
                            // implies: the align-info pass adds nothing and a
                            // fraction-of-a-read counts as a fraction.
                            bv.count[nuc_idx] += weight;
    ...
    fn find_candidate_variants(&mut self, consensus: &[Vec<u8>]) {
        ...
        for i in 0..seq_cnt {
            ...
            let factor = 0.5;
            for j in 0..len {
                // DIVERGENCE from T1K (see docs/DIVERGENCES.md): T1K's
                // `nucToNum` table maps non-ACGT bases (e.g. `N`) to -1 and then
                // indexes `count[-1]` as silent out-of-bounds UB
                // (`VariantCaller.hpp:323`). Reference consensuses do contain
                // `N`s, so `unum` skips those positions -- no variant is ever
                // called against a non-ACGT reference base -- rather than
                // reading undefined memory (or, as the naive port would,
                // panicking).
                let Some(ref_nuc_idx) = nuc_to_num(s[j]) else {
                    continue;
                };
        ...
        for k in 0..=1 {
            ...
            for frag in fragment_assignment {
                ...
                for &op in align {
                    ...
                    //
                    // CONFIRMED REACHABLE on the real `unum analyze`
                    ...
                    // (`unum_core::align_algo::global_alignment(b"ACGTACGTAC",
...
mod tests {
    ...
    #[test]
    fn update_base_variant_weight_below_one_uses_weight_for_count() {
        let refs = vec![allele_ref("ACGT")];
        let mut vc = VariantCaller::new(&refs);
        let read = b"ACGT";
        let o = overlap_no_align(0, 0, 3, 0, 3, 8, 1.0);
        ...
        // `count` accumulates the fractional weight (0.5), while `uniq_count`
        // (weight-1-only) stays 0 and `unweighted_count` is the raw tally (1).
        assert_eq!(vc.base_variants[0][0].count[a_idx], 0.5);
        assert_eq!(vc.base_variants[0][0].uniq_count[a_idx], 0.0);
        assert_eq!(vc.base_variants[0][0].unweighted_count[a_idx], 1.0);
    }

    #[test]
    fn update_base_variant_weight_zero_contributes_no_support() {
        // The align-info-only pass (`update_type == 1`) drives `weight == 0.0`;
        // it must not add any weighted support to `count`.
        let refs = vec![allele_ref("ACGT")];
        let mut vc = VariantCaller::new(&refs);
        let read = b"ACGT";
        let o = overlap_no_align(0, 0, 3, 0, 3, 8, 1.0);
        let consensus = [b"ACGT".to_vec()];
        vc.update_base_variant_from_overlap(read, 0.0, false, &o, &consensus[0]);
        let a_idx = nuc_to_num(b'A').unwrap();
        assert_eq!(vc.base_variants[0][0].count[a_idx], 0.0);
        // The raw tally still records the observation.
        assert_eq!(vc.base_variants[0][0].unweighted_count[a_idx], 1.0);
    }
    ...
    #[test]
    fn find_candidate_variants_skips_non_acgt_consensus_base() {
        // Consensus position 2 is `N`: T1K reads count[nucToNum['N']] =
        // count[-1] (out-of-bounds UB); `unum` skips it. Position 5 is a normal
        // `A` with strong alt support and MUST still be discovered, proving the
        // skip is targeted rather than aborting the whole scan.
        let refs = vec![allele_ref("ACNGTAACGT")]; // len 10; pos 2 = 'N', pos 5 = 'A'
        let mut vc = VariantCaller::new(&refs);
        let consensus = vec![b"ACNGTAACGT".to_vec()];

        let t_idx = nuc_to_num(b'T').unwrap();
        // Strong alt (`T`) support at both the `N` position and a normal `A`.
        vc.base_variants[0][2].count[t_idx] = 10.0;
        vc.base_variants[0][5].count[t_idx] = 10.0;

        // Must not panic on the `N` position (the regression under test).
        vc.find_candidate_variants(&consensus);

        assert!(
            !vc.candidate_variants.contains(&(0, 2)),
            "no candidate should be registered against a non-ACGT reference base"
        );
        assert!(
            vc.candidate_variants.contains(&(0, 5)),
            "a candidate at a normal ACGT position must still be discovered"
        );
    }
