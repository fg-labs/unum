...
/// Regression: `CppGenotyper::parse_allele_name` must size its output buffers
/// from the input length, not a fixed scratch size. The C++ `ParseAlleleName`
/// does `strcpy(gene, allele); strcpy(majorAllele, allele)`, writing
/// `strlen(allele) + 1` bytes before any Rust-side length check could run, so
/// an allele longer than the old fixed 4096-byte buffer overflowed it. Drive an
/// allele longer than 4096 chars through the shim and confirm it neither
/// overflows nor diverges from the Rust port.
#[test]
fn parse_allele_name_long_allele_does_not_overflow_shim_buffer() {
    let allele = format!("KIR2DL1*{}", "0".repeat(5000));
    assert!(allele.len() > 4096, "test allele must exceed the old fixed buffer size");
    let (rust_gene, rust_major) = parse_allele_name(&allele, 0, -1, b'\0');
    let (cpp_gene, cpp_major) = CppGenotyper::parse_allele_name(&allele, 0, -1, b'\0');
    assert_eq!(rust_gene, cpp_gene, "gene mismatch on long allele");
    assert_eq!(rust_major, cpp_major, "majorAllele mismatch on long allele");
}
