    ...
    for seed in 0u64..150 {
        ...
        let read: Vec<u8> = match category {
            ...
            3 => {
                let rec_idx = rng.next_range(seq_count);
                let (_, seq) = &pair.records[rec_idx];
                // Clamp the floor to `seq.len()` for records shorter than 20bp:
                // a plain `.max(20)` could leave `len > seq.len()`, underflowing
                // the `seq.len() - len` below (usize subtract-with-overflow panic).
            ...
            _ => {
                ...
                let (_, seq) = &pair.records[rec_idx];
                // Clamp the floor to `seq.len()` for records shorter than 20bp
                // -- see the delete arm above for why a plain `.max(20)` underflows.
                let len = len.min(seq.len()).max(seq.len().min(20));
                let max_start = seq.len() - len;
                let start = if max_start == 0 { 0 } else { rng.next_range(max_start + 1) };
                let base = &seq[start..start + len];
