...
fn main() {
    ...
    // htslib that `rust-htslib` links into the Rust side -- building the
    // oracle against it (rather than the old bundled samtools-0.1.19) means
    // exactly one htslib copy exists in any process that links both the
    // oracle's `libbam`-equivalent code and rust-htslib, eliminating the
