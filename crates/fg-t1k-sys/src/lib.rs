...
mod ffi {
    ...
    unsafe extern "C" {
        ...
        pub fn fg_t1k_alignments_get_read_seq(p: *mut c_void, buffer: *mut c_char) -> i32;
        pub fn fg_t1k_alignments_get_qual(p: *mut c_void, buffer: *mut c_char) -> i32;
        pub fn fg_t1k_alignments_get_read_id(p: *mut c_void, buffer: *mut c_char) -> i32;
...
/// Buffer size for `fg_t1k_genotyper_parse_allele_name`'s `out_gene`/
/// `out_major` params: generous headroom over any realistic KIR/HLA allele
/// name (the real T1K CLI itself uses a 256-byte `char alleleName[256]`
/// buffer for whitelist parsing, `Genotyper.hpp:686`).
#[cfg(feature = "t1k-sys")]
const GENOTYPER_NAME_BUFFER_SIZE: usize = 4096;
...
impl CppGenotyper {
    ...
    /// # Panics
    ///
    /// Panics if `allele` contains an interior NUL byte, if the underlying
    /// C++ call threw, or if either output string does not fit this
    /// wrapper's internal buffer (generous for any realistic allele name;
    /// see [`GENOTYPER_NAME_BUFFER_SIZE`]).
    #[must_use]
    pub fn parse_allele_name(
        ...
        let mut out_gene = vec![0u8; GENOTYPER_NAME_BUFFER_SIZE];
        let mut out_major = vec![0u8; GENOTYPER_NAME_BUFFER_SIZE];
        ...
        let gene = c_buf_to_vec(&out_gene);
        let major = c_buf_to_vec(&out_major);
        assert!(
            gene.len() < GENOTYPER_NAME_BUFFER_SIZE && major.len() < GENOTYPER_NAME_BUFFER_SIZE,
            "fg_t1k_genotyper_parse_allele_name output did not fit the \
             {GENOTYPER_NAME_BUFFER_SIZE}-byte buffer"
        );
        (
            String::from_utf8(gene).expect("gene must be valid UTF-8"),
            String::from_utf8(major).expect("majorAllele must be valid UTF-8"),
        )
    }
